DROP SCHEMA IF EXISTS wrattel CASCADE;
...
CREATE TABLE person (
    ...
    year_of_study SMALLINT CHECK(year_of_study IN(1,2,3,4,5)),
...
CREATE TABLE supervisor (
    id INT PRIMARY KEY,
    role VARCHAR(20) NOT NULL CHECK(role IN('مسمع','مساعد', 'مشرف', 'سبر')),
...
ADD CONSTRAINT suprevisor_assistant_not_equal CHECK(assistant_id IS NULL OR assistant_id <> id);
...
CREATE TABLE achievement (
    id INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    part SMALLINT CHECK(part BETWEEN 1 AND 30),
    qty SMALLINT CHECK(qty BETWEEN 1 AND 30),
